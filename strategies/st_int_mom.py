...
class int_mom(object):
		...
		for i in range(0,len(candidates)):
			...
			if abs(impact_cost) <= 0.0025:
				...
				if candidates[i]['limit_order_price'] <= self.buy_price[candidates[i]['tradingsymbol']]:
					logging.info("LTP is in buy range. LTP: {ltp}".format(ltp=candidates[i]['limit_order_price']))
				else:
					logging.info("LTP is not within buy range. LTP: {ltp}".format(ltp=candidates[i]['limit_order_price']))
					del_idx.append(i)
			else:
	...
	def add_eod_data_helper(self):
		...
		to_date = datetime.date(2021,5,20)
